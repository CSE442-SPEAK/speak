...
getSignaturesOfPetition:function(petition_id, callback) {
    return db.query("select user.email from user inner join (select * from signature where signature.petition_id=?) as temp on user.user_id=temp.user_id", [petition_id], callback);
},
